...
def enrich_fetched_notables(service, incidents, num_enrichment_events, cache_object):
    ...
    for notable in notables[:MAX_SUBMIT_NOTABLES]:
        task_status = enrich_fetched_notable(service, notable, num_enrichment_events)
        if task_status:
...
def enrich_fetched_notable(service, notable, num_enrichment_events):
...
def handle_open_enrichments(service, enrichment_timeout, incidents, cache_object):
    ...
    for notable in notables[:MAX_HANDLE_NOTABLES]:
        task_status = handle_open_enrichment(service, notable, enrichment_timeout)
        if task_status:
            incidents.append(notable.to_incident())
...
def handle_open_enrichment(service, notable, enrichment_timeout):
    ...
    if not is_enrichment_exceeding_timeout(notable, enrichment_timeout):
...
def is_enrichment_exceeding_timeout(notable, enrichment_timeout):
    ...
    """
    # TODO: think of exception here: min() arg is an empty sequence
    earliest_enrichment_datetime = min(
        datetime.strptime(enrichment.creation_time, JOB_CREATION_TIME_FORMAT)
        for enrichment in notable.enrichments if enrichment.status == Enrichment.IN_PROGRESS
    )
...
def fetch_incidents(service, enrichment_timeout, num_enrichment_events):
    # TODO: go over all docstrings and comments
...
def run_enrichment_mechanism(service, enrichment_timeout, integration_context, num_enrichment_events):
    ...
    1. We first handle open enrichments that have not been handled in the last fetch run
    ...
    try:
        ...
        enrich_fetched_notables(service, incidents, num_enrichment_events, cache_object)
